...
Opis : przeladowanie tabel ze stage i wypełnianie dodatkowych kolumn (np CzyMIG)
...
SET CzyMIG = 
	CASE 
		...
		THEN 2
		--3 jak będzie algorytm rozliczeń
	ELSE 0
	END
...
WHERE 	len(kor_nazwa) > 0	
		AND TRIM(dbo.UsuwanieNieliter(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(nazwa,'SP. KOMANDYTOWA', 'SP. K.'),'SPÓŁKA KOMANDYTOWA', 'SP. K.'),'PRZEDSIĘBIORSTWO HANDLOWO USŁUGOWE','P.H.U'),'SPÓŁKA AKCYJNA','S.A.'),'SPÓŁKA Z O.O.','SP. Z O.O.'),'SPÓŁKA Z OGRANICZONĄ ODPOWIEDZIALNOŚCIĄ','SP. Z O.O.'))) 
			= TRIM(dbo.UsuwanieNieliter(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(kor_nazwa,'SP. KOMANDYTOWA', 'SP. K.'),'SPÓŁKA KOMANDYTOWA', 'SP. K.'),'PRZEDSIĘBIORSTWO HANDLOWO USŁUGOWE','P.H.U'),'SPÓŁKA AKCYJNA','S.A.'),'SPÓŁKA Z O.O.','SP. Z O.O.'),'SPÓŁKA Z OGRANICZONĄ ODPOWIEDZIALNOŚCIĄ','SP. Z O.O.'))) 
		AND kodpocztowy = kor_kod_poczt
		AND miejscowosc = kor_miejsc
		AND ulica = kor_ulica
		AND nrdomu = kor_dom

/** Uzupełnienie LIDERA PPE dla sumatorów (fikcyjne umowy w HM) npr = 0 **/
;
WITH A AS
(SELECT
