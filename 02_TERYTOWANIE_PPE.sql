...
UPDATE NCB_MIG.en.Stg_PPE
	SET TERYT_MIEJSCOWOSC =
		...
		WHERE MIEJSCOWOSC_TMP_CLR = MIEJSCOWOSC_CLR
			AND (TERYT_MIEJSCOWOSC IS NULL OR LEN(TERYT_MIEJSCOWOSC) > LEN(TERYT_MIEJ_SIMC))
			AND LEFT(mscgus,6) = GUS_TMP
			AND LEFT(kod_p,4) = PNA
			AND CzyMIG > 0;
...
UPDATE en.Stg_PPE
	...
	WHERE en.Stg_PPE.MIEJSCOWOSC_CLR = ref.MIEJSCOWOSC_TMP_CLR 
		AND LEFT(en.Stg_PPE.kod_p,4) = ref.pna
...
UPDATE NCB_MIG.en.Stg_PPE
	SET TERYT_MIEJSCOWOSC = dbo.TERYT_Miasta_z_odcinkow(TERYT_MIEJSCOWOSC,ULICE_CLR,dom)
		WHERE CzyMIG >0
				AND LEN(TERYT_MIEJSCOWOSC) > 7
				AND LEN(ULICE_CLR) > 0
				AND LEN(dom) > 0
				AND LEN(TERYT_MIEJSCOWOSC) > LEN(dbo.TERYT_Miasta_z_odcinkow(TERYT_MIEJSCOWOSC,ULICE_CLR,dom))
...
UPDATE NCB_MIG.en.Stg_PPE
	SET TERYT_MIEJSCOWOSC = dbo.TERYT_POD(TERYT_MIEJSCOWOSC,kod_p,-1)
		WHERE 
				CzyMIG > 0
				AND LEN(TERYT_MIEJSCOWOSC)> 7
				AND LEN(TERYT_MIEJSCOWOSC) > LEN(dbo.TERYT_POD(TERYT_MIEJSCOWOSC,kod_p,-1))
